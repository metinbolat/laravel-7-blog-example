                            ...
                            <table class="table table-hover">
                                ...
                                @foreach ($comments as $comment)
                                    <tr>
                                        ...
                                        <td width="5"><form id="{{'form-recover-'.$comment->id}}" method="post" action="{{route('comment_recover',$comment)}}">@method('delete') @csrf<button onclick="event.preventDefault();
                                                           {
                                                    document.getElementById('form-recover-{{$comment->id}}')
                                                    .submit()
                                                    }" class="btn btn-primary"><i class="fa fa-undo"></i></button></form></a></td>
                                        <td width="5"><form id="{{'form-delete-'.$comment->id}}" method="post" action="{{route('comment_destroy',$comment)}}">@method('delete') @csrf<button onclick="event.preventDefault();
